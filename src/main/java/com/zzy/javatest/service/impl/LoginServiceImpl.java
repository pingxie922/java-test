...
public class LoginServiceImpl implements LoginService {
	...
	public Map<String, Object> login(User user) {
		...
		try {
			if(StringUtils.isEmpty(user.getTelphone())) {
				ResultMap.error(map, "请输入账号");
				return map;
			}
			if(StringUtils.isEmpty(user.getPassword())) {
				ResultMap.error(map, "请输入密码");
				return map;
			}
			...
			if(loginUser != null) {					 						//登陆成功，保存登陆日志
				...
				ResultMap.success(map, "登陆成功", loginUser);
				...
				ResultMap.error(map, "账号或密码错误，请重试");
			}
		} catch (Exception e) {
			ResultMap.error(map, "登陆异常，请稍后重试");
			log.info("登陆异常了");
			e.printStackTrace();
		}
		return map;
	...
	public Map<String, Object> reg(User user, String phonecode) {
		...
		try {
			if(StringUtils.isEmpty(user.getName())) {
				ResultMap.error(map, "用户名为空");
				return map;
			}
			if(StringUtils.isEmpty(user.getTelphone())) {
				ResultMap.error(map, "用户手机号为空");
				return map;
			}
			if(StringUtils.isEmpty(user.getPassword())) {
				ResultMap.error(map, "用户密码为空");
				return map;
			}
			if(StringUtils.isEmpty(phonecode)) {
				ResultMap.error(map, "手机验证码为空");
				return map;
			}
			...
			if(StringUtils.isEmpty(phone_code)) {
				ResultMap.error(map, "手机验证码已过期");
				return map;
			}
			if(!phone_code.equals(phonecode)) {
				ResultMap.error(map, "手机验证码错误");
				return map;
			}
			...
			if(i) {
				...
				ResultMap.success(map, "注册成功,更多内容容我开发", user);
			} else {
				ResultMap.error(map, "注册失败，请稍后重试");
			}
		} catch (Exception e) {
			log.info("注册异常");
			ResultMap.error(map, "注册失败，请稍后重试");
	...
	public Map<String, Object> isExistByTelPhone(String telPhone) {
		...
		try {
			...
			ResultMap.success(map, "可用", "注册手机验证码 :" + random);
		} catch (Exception e) {
			log.info("获取手机验证码异常");
			ResultMap.error(map, "发送验证码失败，请稍后重试");
			e.printStackTrace();
		}
		return map;
	}
	...
	public Map<String, Object> getHomeStateMent() {
		...
		try {
			...
			if(one != null) {
				log.info("获取的语句为:" + one);
				ResultMap.success(map, "获取成功", one);
			}
		} catch (Exception e) {
			log.info("获取首页语句出错");
			ResultMap.error(map, "发送验证码失败，请稍后重试");
